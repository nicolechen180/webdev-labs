...
async def respond_to_message(websocket, message):
    ...
    # Goal: let's respond to different kinds of messages differently:
    type = data.get('type')

    # DONE: login to Web socket server
    if type == 'login':
    ...
    # DONE: When any of the clients sends a message of type "chat", 
    ...
    # DONE we've done the disconnect:
        ...
        # send a message to all of the users that a client has left:
        for socket in logged_in_users:
            await socket.send(json.dumps(message))
    else:
        print('Read the instructions to format your message correctly')
    '''
    ******************************************************************
